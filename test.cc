...
/*
  ...
  corpus<double, std::string> pp0, pp1;
  pp0.init(words0, 0, 120);
  pp1.init(words0, 0, 120);
  pp0.compute(p0, delimiter);
  pp1.compute(p1, delimiter);
...
*/
  ...
  auto input(loadbuf(argv[2]).second);
  switch(mode) {
  case 0:
    // lword
    {
      if(3 <= argc) {
        auto elimlist(cutText(loadbuf(argv[3]).second, csvelim, csvdelim));
  ...
  case 12:
    ...
    {
      const auto words0(cutText(loadbuf(argv[3]).second, csvelim, csvdelim));
      ...
      for(int i = 0; i <= input.size() / szblock; i ++)
        std::cout << preparedTOC<double, std::string>(input.substr(i * szblock, szblock), std::string("ref") + std::to_string(i) + std::string("-"), words0, detailwords, details, tocwords, tocs, delimiter, szwindow, double(.125), threshin, .125, mode == 12) << std::string("<hr/>") << std::endl;
